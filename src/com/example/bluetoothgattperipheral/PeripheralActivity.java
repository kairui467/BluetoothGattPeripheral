...
public class PeripheralActivity extends Activity {
    ...
    private BluetoothGattServerCallback mGattServerCallback = new BluetoothGattServerCallback() {
        ...
        public void onCharacteristicWriteRequest(BluetoothDevice device,
			...
			AmazingToast.showToast(getApplicationContext(), "value.length : " + value.length, 1);
            
            runOnUiThread(new Runnable() {
				public void run() {
					//AmazingToast.showToast(getApplicationContext(), "value = " + new String(value), 1);
				}
			});
            ...
            if (DeviceProfile.CHARACTERISTIC_OFFSET_UUID.equals(characteristic.getUuid())) {
                ...
                mHandler.post(new Runnable() {
                    @Override
                    public void run() {
                        //Toast.makeText(PeripheralActivity.this, "时间偏移更新", Toast.LENGTH_SHORT).show();
                    }
                });
    ...
    private void notifyConnectedDevices() {
        for (BluetoothDevice device : mConnectedDevices) {
            ...
            readCharacteristic.setValue(new byte[]{11});
